    ...
    _configs()
{
  ...
  for ( int i = 0; i < 4; i++ ) {
    ...
    boost::random::uniform_real_distribution<> link_queue_size( range.link_limit.first, range.link_limit.second );
    ...
    printf("Adding new configuration: %s", _configs.back().str().c_str());
  }
}
